...
class MultiscaleSolver:
	...
	def AddWalkArea(self,area):
		self.WalkSolvers.append(Walk(area,M=100))
	...
	def SaveState(self,path=None,format='npy',fname=None):
		t = time.gmtime()
		datetime = '%02d%02d%d_%02d%02d'%(t.tm_mday,t.tm_mon,t.tm_year,t.tm_hour,t.tm_min)
		filename = 'Results_%s_%04d'%(datetime,self.IterationCounter) if fname is None else '%s_%04d'%(fname,self.IterationCounter)
		if path is not None:
