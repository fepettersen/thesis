...
void Combine::ConvertFromWalkers(double **u, int**Conc, int **index){
	...
	if(d==1){
		...
		double y1,y2;
		y1 = (u[m0][0]-u[m0-1][0])/dx;
		y2 = (u[m0+M][0]-u[m0+1+M][0])/dx;
		double v[M];
		CubicSpline(tmp,v,M,y1,y2);
		for(int k=0;k<M;k++){
			u[k+m0][0] = v[k];
		}
		// delete [] tmp,v;
...
void Combine::CubicSpline(double *y, double *v, int M, double yp1, double yp2){
	...
	if(yp1>INFINITY) {v[0] = bu[0] = 0.0;}
	else {
		v[0] = -0.5;
		bu[0] = (3.0/dx)*((y[1]-y[0])/dx - yp1);
	}
	for(i=0; i<M-1; i++){
		sig = 0.5;
		p = sig*v[i-1] +2.0;
		v[i] = (sig-1)/p;
		bu[i] = (y[i+1]-y[i])/dx - (y[i]-y[i-1])/dx;
		bu[i]  = (6.0*bu[i]/(2*dx) - sig*bu[i-1])/p;
	}
	if(yp2 > INFINITY)  qn = un = 0.0;
    else {
      qn = 0.5;
      un = (3.0/dx)*(yp2 - (y[M-1] - y[M-2])/dx);
   }
   for(k=(M-2); k>=0; k--){
   	v[k] = v[k]*v[k+1] + bu[k];
   }
