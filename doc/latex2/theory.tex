...
\end{equation}
The number of walkers leaving $V$ can also be described by the number of walkers crossing the surface $S$ in the outward direction, $n$ by integrating the component of the flux of walkers, which is the number of walkers going in a \textcolor{red}{certain direction} per time per area, normal to the surface $S$ over the entire surface.
\begin{equation}
 \int_S\vec{J}\cdot \vec{n}\,dS
\end{equation}
Since no walkers are created or disappear, the two integrals must be equal.
