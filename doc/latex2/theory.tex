...
As the random walkers start to move some of them will leave the volume $V$ which results in a change in $C$. 
...
The flux $\vec{J}$ can be expressed by Fick's first law as the diffusive flux 
\begin{equation}
 \vec J = D\nabla C
\end{equation}

where $D$ is the diffusion constant. Since $V$ was chosen as an arbitrary volume equation \eqref{theory:integral_form} is independent of the integration and the integrals can be dropped. 
...
Equation \eqref{general_scheme_BE2D} assembles to the linear system in equation \eqref{linear_system_BE2D} when solved on a $3\times3$ mesh.
...
All entries in eq. \eqref{block_tridiagonal_dashed} are $3\times3$ matrices. 
...
\begin{equation}\label{theory_step_length}
