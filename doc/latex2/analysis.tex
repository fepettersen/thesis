...
% Error plots for FE in 1D (a) and 2D (b)
 ...
 \end{subfigure}
 \begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{../results/experiment_29112013_1709/results/deterministic_errorplot.eps}
  \caption{}
 \end{subfigure}
 \caption[]{}
...
\end{equation}
\noindent $H(x)$ denotes the Heaviside step function defined by its properties
...
Figure \ref{ConvergenceTestRW:Hc} verifies that the error term improves by adding more walkers. And figure \ref{ConvergenceTestRW:dt} shows that the RW model will fulfill the chosen time step on the PDE level. 
The convergence is not very good, but this is expected because of the fluctuations which causes the error to stabilize around a certain value rather than converge to zero.
