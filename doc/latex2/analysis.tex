...
\begin{itemize}
 ...
 By adjusting the source term, $s$, one can, in principle, choose any solution to the diffusion equation. 
...
Notice that there is one term arising from the time derivative and one from the spatial derivative and that they are of different order. \\
Where possible, convergence will be tested by having $\Delta t \gg \Delta x^2$, which implies $\Delta x\approx0$ and 
$$
\epsilon \propto \Delta t^r
$$
where $r=1$. 
However, for the FE scheme $\Delta t \gg \Delta x^2$ will violate the stability criterion, and a single discretization parameter, $h$, must be introduced by ensuring  
\begin{equation*}
...
\begin{align*}
 ...
 &= (C_t +C_x/k)h = C_k h^r
\end{align*}
As $\Delta t$ or $h$ tends to zero, $r$ is expected to converge to one, and that is the purpose of these tests. 
