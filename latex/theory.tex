...
Insertion of the Gaussian distribution \ref{rw_gaussian_distribution}) verifies that the Gaussian distribution fulfills the diffusion equation. 
...
Simply replacing the diffusion constant by a function $D = D(\vec{x})$ (see eq \ref{steplength}) is a natural first approximation, but this will not quite be sufficient as Farnell and Gibson point out \cite{farnell2005monte}. 
...
\end{equation}
The adjusted jump probabilities $T_r(x)$ and $T_l(x)$ which are the probabilities for a walker at position x to jump right or left, respectively are defined in equations \ref{farnell_and_gibson_4} and \ref{farnell_and_gibson_5}
...
To maintain a bit of generality we will look at the (potentially) anisotropic diffusion equation in 2d. The extension to 3d is trivial, as is the 1d version. The drift term will be omitted in the beginning because this also results in a simple addition to the numerical scheme. The equation to be discussed is 
\begin{equation}
 \frac{\d u}{\d t} = \nabla D\nabla u +f
\end{equation}
...
From the theta rule we can see that the only difference between the FE and BE scheme is at what time-step the right-hand-side of the equation is evaluated. 
